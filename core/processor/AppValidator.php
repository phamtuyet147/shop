...
final class AppValidator
{
    ...
    public static function init($validationFiles)
    {
        ...
        if (is_null(self::$validationFiles)) {
            self::$validationFiles = array();
        }
        if (empty(self::$CUSTOM_RULES)) {
            $customFile = W_CORE . DIRECTORY_SEPARATOR . self::CUSTOM_RULE_FILE;
            $customRules = FileUtils::readXMLFile(
                $customFile, self::NAMESPACE_PREFIX
            );
    ...
    public static function initForms()
    {
        ...
        if (!$forms) {
            $forms = FileUtils::readMultipleXMLFile(
                self::$validationFiles, 'forms', AppInfo::$BASE_PATH
            );
        ...
        $forms->registerXPathNamespace(
            self::NAMESPACE_PREFIX, self::VALIDATION_NAMESPACE
        );
        self::$FORMS = $forms;
    }
    ...
    public static function getField($form, $name)
    {
        ...
        $forms = self::$FORMS;
        $field = $forms->xpath(
            "//{$prefix}:form[@name='$form']/{$prefix}:field[@name='{$name}']"
        );
        if (!$field) {
            return null;
        }
